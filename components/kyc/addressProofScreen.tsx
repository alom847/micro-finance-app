...
import { UpdateAddressProof } from "../../constants/api";
...
const DocTypes = [
  { key: "1", value: "Voter ID" },
  { key: "2", value: "Adhaar" },
  { key: "3", value: "Driving License" },
  { key: "4", value: "Ration Card" },
  { key: "5", value: "Other" },
];
...
const IDProofScreen = ({ onSubmit }: { onSubmit: () => void }) => {
  ...
  const onStepSubmit = async () => {
    ...
    body.append("address_proof_type", docType);
    body.append("address_proof_value", docValue);

    let filename = front.split("/").pop() as string;
    let match = /\.(\w+)$/.exec(front);
    let type = match ? `image/${match[1]}` : `image`;

    body.append("address_proof_front", {
      uri: front,
      name: filename,
      type,
    } as any);
    ...
    body.append("address_proof_back", {
      uri: back,
      name: filename,
      type,
    } as any);
    ...
    setLoading(true);
    try {
      const res = await handleApiResponse(UpdateAddressProof, [body]);
